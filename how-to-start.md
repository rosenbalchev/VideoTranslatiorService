...
| `ERROR: No matching distribution found for torch==2.5.1` (Windows on ARM only) | Python 3.12 is the **ARM64** build, but `torchaudio` (which we also require) has never published `win_arm64` wheels — only `win_amd64`. `torch` itself gained native win_arm64 wheels at 2.7.0, but that doesn't help since torchaudio still has none; there's no version combination that works natively | `install.bat` now detects an ARM64 Python interpreter and fails fast with instructions, instead of letting pip fail deep into the install. Fix: install the **x64** build of Python 3.12 instead (`winget uninstall -e --id Python.Python.3.12` then `winget install -e --id Python.Python.3.12 --architecture x64`) — it runs fine under Windows' built-in x64 emulation, and our existing win_amd64 pins then resolve normally. Delete any existing venv (`scripts\uninstall.bat`) and re-run `install.bat` afterwards |
| `ERROR: No matching distribution found for torch==2.5.1+cu124` (Linux on ARM64/aarch64 with an NVIDIA GPU — Jetson, GH200/Grace-Hopper) | torch/torchvision/torchaudio 2.5.1 publish plain CPU wheels for `linux_aarch64`, but never compiled a `+cu124` CUDA build for that architecture. `nvidia-smi` succeeding is what triggers `install.sh`'s CUDA auto-detection, so this only bites on real ARM64 hardware with an NVIDIA GPU present | `install.sh` now detects `uname -m` = `aarch64`/`arm64` on Linux and forces CPU mode automatically (same pattern as the macOS `--cuda` override), rather than letting pip fail. CPU mode works fine on Linux ARM64 — the plain wheels exist. Real GPU acceleration on Jetson/GH200 needs NVIDIA's own separate wheel channel (not `download.pytorch.org`), which is out of scope here |
