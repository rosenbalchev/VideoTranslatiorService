# RB.VideoTranslator.Core NuGet Package

This document describes how to use the `RB.VideoTranslator.Core` NuGet package to run the video translation pipeline.

## Installation

The package is available locally at `./nupkg/RB.VideoTranslator.Core.1.0.0.nupkg` (it depends on `RB.VideoTranslator.Domain`, published alongside it).

To install:
```
dotnet add package RB.VideoTranslator.Core --source ./nupkg
```

Or reference it directly in your `.csproj`:
```xml
<PackageReference Include="RB.VideoTranslator.Core" Version="1.0.0" />
```

## Quick Start

The package exposes a public `IPipelineRunner` interface (from `RB.VideoTranslator.Domain`) that is the main entry point for consumers:
...
The pipeline is configured via `PipelineOptions` (`RB.VideoTranslator.Domain.Models`):
...
Configuration class with all pipeline settings (see Configuration section above). Lives in `RB.VideoTranslator.Domain.Models`.

### ConfigurationMerger Class

Static helper (`RB.VideoTranslator.Core`) for merging CLI-style options into PipelineOptions programmatically:
...
- RB.VideoTranslator.Domain (enums, constants, interfaces, DB entities, and public models)
...
- `RB.VideoTranslator.Core.dll` - Main library assembly
- All dependent assemblies (including `RB.VideoTranslator.Domain.dll`)
